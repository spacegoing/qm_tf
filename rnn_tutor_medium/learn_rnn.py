...
def generateData():
    ...
    x = x.reshape((batch_size,
                   -1))  # The first index changing slowest, subseries as rows
...
for current_input in inputs_series:
    ...
    input_and_state_concatenated = tf.concat([current_input, current_state],
                                             1)  # Increasing number of columns

    next_state = tf.tanh(
        tf.matmul(input_and_state_concatenated, W) + b)  # Broadcasted addition
...
def plot(loss_list, predictions_series, batchX, batchY):
    ...
    for batch_series_idx in range(5):
        ...
        single_output_series = np.array([(1 if out[0] < 0.5 else 0)
                                         for out in one_hot_output_series])
